...
class PairingApi {
  ...
  start() {
    ...
    this.app.get('/health', (_, res) => res.json({ ok: true, version: '0.8.1', mode: 'discord-only' }));
    ...
    this.app.post('/api/device-pairing/:code/complete', async (req, res) => {
      try {
        const result = await this.pairing.completeDeviceTicket(String(req.params.code || ''), req.body);
        res.json({ ok: true, result });
      } catch (err) {
        res.status(400).json({ ok: false, error: err.message || 'Device pairing failed' });
      }
    });
